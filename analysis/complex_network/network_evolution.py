...
def render():
    ...
    if incomplete:
        ...
        intro += (
            ...
            f'⚠️ <b>数据空洞 · Data gap:</b> 快照 '
            f'<code>{joined}</code> 的 IYP dump 未包含 '
            f'<code>(:Prefix)-[:CATEGORIZED]->(Tag "RPKI Valid")</code> 与 '
            f'<code>[:DEPENDS_ON]</code> 关系，'
            f'④ RPKI% 与 ① Dependency edges 读作 0 — 非真实下跌 '
            f'(③ Top-10 prefix share 使用的是 :Prefix 节点，不受影响)。'
            f'<br>Snapshot(s) <code>{joined}</code>: the dump lacks RPKI '
            f'Tag relationships and AS dependency edges; panels ④ and the '
            f'dep-edge series of ① show 0 — not real drops.</p>'
        )
